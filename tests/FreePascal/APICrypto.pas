...
==========
 ...
 This is a test unit for the Crypto functions within the Ultibo RTL.

 To run these tests create a new project, create an HTTPListener and
 add the WebStatus unit (see the examples for more information).

 Using a web browser, browse to the Web Status page (http://<ipaddress>/status)
 and scroll down the list on the left hand side until you find the
 Crypto API entry. Click on the Crypto API link and the tests will execute.

 The details of each test along with the results will be reported in
 the browser for verification.
...
begin
 {}
 inherited Create('Crypto API Tests','/cryptoapi',2)
end;
...
begin
 ...
 if MD5DigestString('abc',@MD5Digest) then
  begin
   ...
   Expected:=Lowercase('900150983CD24FB0D6963F7D28E17F72'); {Source: https://www.nist.gov/itl/ssd/software-quality-group/nsrl-test-data}
 ...
 if MD5DigestString('abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq',@MD5Digest) then
  begin
   Actual:=MD5DigestToString(@MD5Digest);
   Expected:=Lowercase('8215EF0796A20BCAAAE116D3876C664A'); {Source: https://www.nist.gov/itl/ssd/software-quality-group/nsrl-test-data}
 ...
 if MD5DigestString(StringOfChar('a',1000000),@MD5Digest) then
  begin
   Actual:=MD5DigestToString(@MD5Digest);
   Expected:=Lowercase('7707D6AE4E027C70EEA2A935C2296F21'); {Source: https://www.nist.gov/itl/ssd/software-quality-group/nsrl-test-data}
 ...
 if SHA1DigestString('abc',@SHA1Digest) then
  begin
   ...
   Expected:=Lowercase('A9993E364706816ABA3E25717850C26C9CD0D89D'); {Source: https://www.nist.gov/itl/ssd/software-quality-group/nsrl-test-data}
 ...
 if SHA1DigestString('abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq',@SHA1Digest) then
  begin
   Actual:=SHA1DigestToString(@SHA1Digest);
   Expected:=Lowercase('84983E441C3BD26EBAAE4AA1F95129E5E54670F1'); {Source: https://www.nist.gov/itl/ssd/software-quality-group/nsrl-test-data}
 ...
 if SHA1DigestString(StringOfChar('a',1000000),@SHA1Digest) then
  begin
   Actual:=SHA1DigestToString(@SHA1Digest);
   Expected:=Lowercase('34AA973CD4C4DAA4F61EEB2BDBAD27316534016F'); {Source: https://www.nist.gov/itl/ssd/software-quality-group/nsrl-test-data}
 ...
 if SHA256DigestString('abc',@SHA256Digest) then
  begin
   ...
   Expected:=Lowercase('BA7816BF8F01CFEA414140DE5DAE2223B00361A396177A9CB410FF61F20015AD'); {Source: https://www.nist.gov/itl/ssd/software-quality-group/nsrl-test-data}
 ...
 if SHA256DigestString('abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq',@SHA256Digest) then
  begin
   Actual:=SHA256DigestToString(@SHA256Digest);
   Expected:=Lowercase('248D6A61D20638B8E5C026930C3E6039A33CE45964FF2167F6ECEDD419DB06C1'); {Source: https://www.nist.gov/itl/ssd/software-quality-group/nsrl-test-data}
 ...
 if SHA256DigestString(StringOfChar('a',1000000),@SHA256Digest) then
  begin
   Actual:=SHA256DigestToString(@SHA256Digest);
   Expected:=Lowercase('CDC76E5C9914FB9281A1C7E284D73E67F1809A48A497200E046D39CCC7112CD0'); {Source: https://www.nist.gov/itl/ssd/software-quality-group/nsrl-test-data}
 ...
 {CRC-32 Hash Tests}
 AddBold(AResponse,'CRC-32 Hash Tests','');
 AddBlank(AResponse);

 CRC32Data:='abc';
 AddItemEx(AResponse,'Test:','CRC-32',3);
 AddItemEx(AResponse,'Value:',CRC32Data,3);
 CRC32Value:=CRC32(0,nil,0);
 CRC32Value:=CRC32(CRC32Value,PByte(CRC32Data),Length(CRC32Data));
 Actual:=Lowercase(IntToHex(CRC32Value,8));
 Expected:=Lowercase('352441c2'); {Source: https://github.com/froydnj/ironclad/blob/master/testing/test-vectors/crc32.testvec}
 ...
 CRC32Data:='abcdefghijklmnopqrstuvwxyz';
 AddItemEx(AResponse,'Test:','CRC-32',3);
 AddItemEx(AResponse,'Value:',CRC32Data,3);
 CRC32Value:=CRC32(0,nil,0);
 CRC32Value:=CRC32(CRC32Value,PByte(CRC32Data),Length(CRC32Data));
 Actual:=Lowercase(IntToHex(CRC32Value,8));
 Expected:=Lowercase('4c2750bd'); {Source: https://github.com/froydnj/ironclad/blob/master/testing/test-vectors/crc32.testvec}
 ...
 CRC32Data:='ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
 AddItemEx(AResponse,'Test:','CRC-32',3);
 AddItemEx(AResponse,'Value:',CRC32Data,3);
 CRC32Value:=CRC32(0,nil,0);
 CRC32Value:=CRC32(CRC32Value,PByte(CRC32Data),Length(CRC32Data));
 Actual:=Lowercase(IntToHex(CRC32Value,8));
 Expected:=Lowercase('1fc2e6d2'); {Source: https://github.com/froydnj/ironclad/blob/master/testing/test-vectors/crc32.testvec}
 ...
 CRC32Data:='12345678901234567890123456789012345678901234567890123456789012345678901234567890';
 AddItemEx(AResponse,'Test:','CRC-32',3);
 AddItemEx(AResponse,'Value:',CRC32Data,3);
 CRC32Value:=CRC32(0,nil,0);
 CRC32Value:=CRC32(CRC32Value,PByte(CRC32Data),Length(CRC32Data));
 Actual:=Lowercase(IntToHex(CRC32Value,8));
 Expected:=Lowercase('7ca94a72'); {Source: https://github.com/froydnj/ironclad/blob/master/testing/test-vectors/crc32.testvec}
